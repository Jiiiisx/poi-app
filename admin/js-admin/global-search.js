    ...
    async function loadSearchData() {
        ...
        try {
            if (isProfilePage) {
                ...
                const monitoringRes = await fetchWithAuth(`/backend?action=fetch-monitoring&ranges=${Object.values(salesDataRanges).join(',')}`);
                if (monitoringRes.ok) {
                ...
                const [customerRes, governmentRes, monitoringRes] = await Promise.all([
                    fetchWithAuth('/backend?action=customer-data'),
                    fetchWithAuth('/backend?action=government-data'),
                    fetchWithAuth(`/backend?action=fetch-monitoring&ranges=${Object.values(salesDataRanges).join(',')}`)
                ]);
